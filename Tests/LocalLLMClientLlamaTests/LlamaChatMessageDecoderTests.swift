...
struct LlamaChatMessageDecoderTests {
    ...
    func qwen2_5_VL() async throws {
        ...
        let (selected, chunks) = try validate(chatTemplate: template)
        #expect(selected == .qwen2_5_VL)
    ...
    func llama3_2_V() async throws {
        ...
        let (selected, chunks) = try validate(chatTemplate: template, messages: [
            .user(userMarker, attachments: [.testImage]),
            .assistant(assistantMarker),
        ])
        #expect(selected == .llama3_2V)
    ...
    func validate(chatTemplate: String, messages: [LLMInput.Message]? = nil) throws -> (template: ChatTemplate, chunks: [MessageChunk]) {
...
//            "bos_token": "bos_token",
        ...
        let decoder = LlamaAutoMessageDecoder(chatTemplate: chatTemplate)
        ...
        let result = try decoder.applyTemplate(value, chatTemplate: chatTemplate, additionalContext: specialTokens)
        let chunks = try decoder.extractChunks(prompt: result, imageChunks: [[.testImage]])
        return (decoder.chatTemplate, chunks)
    }
}
